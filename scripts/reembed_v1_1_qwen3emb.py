...
def _embed_ollama_v1(model_name: str, text: str, dimensions: int):
    """v1.1 (2026-07-01): use OpenAI-compat /v1/embeddings so `dimensions`
    (MRL truncation) works for qwen3-embedding. Native /api/embeddings ignores
    dimensions and returns full 4096-dim. Returns L2-normalized vector."""
...
def embed_with_ollama(model_name: str, batch: int, dimensions: int = 1024):
    import urllib.request, json
    pages = sorted((ROOT / "wiki").rglob("*.md"))
    print(f"[reembed-ollama] {len(pages)} pages via Ollama model={model_name} "
          f"@{dimensions if dimensions>0 else 'native'}d")
    # Probe dim
    dim = len(_embed_ollama_v1(model_name, "probe", dimensions))
    ...
    for p in pages:
        ...
        vec = _embed_ollama_v1(model_name, prompt, dimensions)
...
if __name__ == "__main__":
    ...
    ap.add_argument("--dimensions", type=int, default=1024,
                    help="MRL truncation dim for /v1/embeddings (0 = native, bge-m3)")
    args = ap.parse_args()
    if args.ollama:
        embed_with_ollama(args.ollama, args.batch, args.dimensions)
    elif args.model:
