---
title: "Evolutionary Technologies International (ETI)"
slug: evolutionary-technologies
page_type: entity
entity_type: company
sector: "enterprise-software"
status: "[DEFERRED]"
successor: "[DEFERRED]"
years_active: "1980s-2000s"
tier: 2
study_count: 0
obs_count: 0
aliases: ["Evolutionary Technologies International (ETI)"]
tags: [type/entity, sector/enterprise-software, status/deferred]
source_csv: known_entities.csv
---

# Evolutionary Technologies International (ETI)

Evolutionary Technologies International (ETI) is a company in enterprise-software. Active 1980s-2000s. Status: [DEFERRED].

**Appears in 0 studies, 0 observations.**

## DuckDB query for full data

```sql
SELECT * FROM observations WHERE entity_id = 'evolutionary-technologies';
```

