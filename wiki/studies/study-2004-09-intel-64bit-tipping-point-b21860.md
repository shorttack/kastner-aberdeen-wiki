...
- Intel Xeon with EM64T preferable for general-purpose applications: web, mail, digital content creation, mechanical CAD, EDA; mixed 32/64-bit environments `[ps=5]`
...
- IDC: EM64T strategy provides excellent investment protection and additional headroom with few or no drawbacks; will initiate broad move toward 64-bit capable platforms `[ps=5]`
...
- Legacy mode (32-bit OS + apps), Compatibility mode (64-bit OS + 32-bit apps), 64-bit mode (full 64-bit OS + apps) `[ps=4]`
- 64-bit processor transcends 4GB memory limit of 32-bit; can directly access virtually unlimited physical memory; dramatic performance for large memory-intensive applications `[ps=4]`
...
- Performance boost of up to 50% for existing 32-bit applications on EM64T platforms vs prior Xeon platforms `[ps=2]`
- 94% of current Itanium-installed firms plan to buy more over next three years; 50% of non-buyers list it in future spend plans `[ps=2]`
- RFID tags and point-of-sale devices causing quantum leap in processing and data requirements that may dwarf end-user workloads `[ps=2]`
...
- Aberdeen Group (Peter Kastner, May 14 2004): Intel Itanium consistently outperforms 64-bit RISC-based servers; Intel-based platforms better performing per processor, more scalable, more cost-effective, and more flexible `[ps=1]`
- SpecjAppServer2002 leading benchmark results for absolute performance and price/performance all achieved using Intel architecture, majority on Itanium 2 `[ps=0]`
