...
- Smart card (NC Card) + password/PIN; higher security than magnetic stripe; enables VPN `[ps=5]`
...
- Based on LDAP; models user/resource mappings; supports charging for resource access `[ps=3]`
- Based on Sun NFS; centralized file storage (NC clients have no local storage) `[ps=3]`
- Key values: low cost of operation, security, scalability to accommodate several thousands of users `[ps=3]`
...
- Integrates with Surfwatch web content filter; future: Oracle Context language processing for dynamic filtering `[ps=2]`
- Integrates with Oracle Financial Application Suite for complete business support solution `[ps=2]`
