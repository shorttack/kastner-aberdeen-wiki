---
title: "Rb Enablingb2Bcollaboration 3890 (Aberdeen, 2007)"
slug: "study-rb-enablingb2bcollaboration-3890-56c184"
page_type: "study"
tags: ["type/study", "collection/benchmark"]
tier: 2
source_csv: "_master_studies.csv"
study_id: "rb-enablingb2bcollaboration-3890-56c184"
author: "Aberdeen Group"
date: "2007-02-14"
pub_year: 2007
type: "benchmark"
subject_domain: "supply-chain"
methodology: "industry-analysis, prepared-candidate-finalization"
source_file: "rb_enablingb2bcollaboration_3890.pdf"
license: "CC-BY-4.0"
importance: "medium"
relevance: "medium"
study_prescience_enum: "low"
prescience_3y_enum: ""
prescience_5y_enum: ""
prescience_max: 3.0
prescience_mean: 1.0
prescience_obs_count: 3
---

# Rb Enablingb2Bcollaboration 3890 (Aberdeen, 2007)

> Aberdeen Group study rb-enablingb2bcollaboration-3890-56c184. Source: rb_enablingb2bcollaboration_3890.pdf. Methodology: industry-analysis, prepared-candidate-finalization. Subject domain: supply-chain. Abstract reconstructed from metadata; original text lost to PDF image-stripper sentinel during ingest.


_Published 2007, author **Aberdeen Group**, type **benchmark**._


## Top observations

- For instance, 76% of Best in Class companies identify working collaboratively with internal organizations and customers as **the most critical element** to a successful demand management program. `[ps=3]`
- Figure 1 shows the maturity levels of B2B collaboration. `[ps=0]`
- For instance, Aberdeen research finds that 73% of companies are doing forecast collaboration with fewer than 10 customers. `[ps=0]`
