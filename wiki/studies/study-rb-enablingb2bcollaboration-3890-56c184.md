---
title: "==> picture [441 x 108] intentionally omitted <=="
slug: "study-rb-enablingb2bcollaboration-3890-56c184"
page_type: "study"
tags: ["type/study", "collection/benchmark"]
tier: 1
source_csv: "_master_studies.csv"
study_id: "rb-enablingb2bcollaboration-3890-56c184"
author: "Aberdeen Group"
date: "2007-02-14"
pub_year: 2007
type: "benchmark"
subject_domain: "supply-chain"
methodology: "industry-analysis, prepared-candidate-finalization"
source_file: "rb_enablingb2bcollaboration_3890.pdf"
license: "CC-BY-4.0"
importance: "medium"
relevance: "medium"
study_prescience_enum: "[DEFERRED]"
prescience_max: 5.0
prescience_mean: 3.0
prescience_obs_count: 3
---

# ==> picture [441 x 108] intentionally omitted <==

> This study analyzes ==> picture [441 x 108] intentionally omitted <== using Aberdeen Group research material and prepared extraction artifacts.


_Published 2007, author **Aberdeen Group**, type **benchmark**._


## Top observations

- For instance, 76% of Best in Class companies identify working collaboratively with internal organizations and customers as **the most critical element** to a successful demand management program. `[ps=5]`
- For instance, Aberdeen research finds that 73% of companies are doing forecast collaboration with fewer than 10 customers. `[ps=4]`
- Figure 1 shows the maturity levels of B2B collaboration. `[ps=0]`
