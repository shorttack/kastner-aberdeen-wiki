...
- Each card deck submission was a financial event; careless programs failing on first run wasted real money, not just time. `[ps=3]`
- Mastering JCL separated people who understood the system from those who merely used it—not glamorous but critical knowledge. `[ps=3]`
- Kastner internalized from Siegel that a programmer's job is choreographing hardware, not solving mathematical problems. `[ps=3]`
- An applications programmer manages physical machine resources: disk spin, core memory cycle, card reader click. Code is choreography of hardware. `[ps=2]`
- Kastner joined PHI in March 1969 as a 21-year-old junior programmer earning $7,200/year. `[ps=0]`
- Converted funeral home in Arlington, Massachusetts; atmosphere half dormitory, half NASA mission control. `[ps=0]`
- Robert A. Siegel later architected the Wang VS operating system after leaving PHI. `[ps=0]`
