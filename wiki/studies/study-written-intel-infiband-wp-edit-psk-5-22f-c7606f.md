...
- The InfiniBand architecture consists of the following three main components (Figure 1): `[ps=0]`
- Figure 2 shows an example of how this existing I/O infrastructure is implemented in today’s data center. `[ps=0]`
- As mentioned earlier, today's approach to storage uses a separate interface per server to access each of the varied storage platforms (Figure 2). `[ps=0]`
- Figure 3 illustrates a reference design for a blade-server rack containing 12 processing blades with a redundant switched backplane and common power. `[ps=0]`
