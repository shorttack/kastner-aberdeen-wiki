---
title: "==> picture [442 x 39] intentionally omitted <=="
slug: "study-written-webex-taking-conferencing-to-the-business-communications-summit-2b6842"
page_type: "study"
tags: ["type/study", "collection/market-study"]
tier: 1
source_csv: "_master_studies.csv"
study_id: "written-webex-taking-conferencing-to-the-business-communications-summit-2b6842"
author: "Aberdeen Group"
date: "2002-01-01"
pub_year: 2002
type: "market-study"
subject_domain: "wireless"
methodology: "industry-analysis, prepared-candidate-finalization"
source_file: "Written__WebEx- Taking Conferencing to the Business Communications Summit.pdf"
license: "CC-BY-4.0"
importance: "medium"
relevance: "medium"
study_prescience_enum: "[DEFERRED]"
prescience_max: 5.0
prescience_mean: 2.5
prescience_obs_count: 2
---

# ==> picture [442 x 39] intentionally omitted <==

> This study analyzes ==> picture [442 x 39] intentionally omitted <== using Aberdeen Group research material and prepared extraction artifacts.


_Published 2002, author **Aberdeen Group**, type **market-study**._


## Top observations

- Market recognition of WebEx’s unique network-driven conferencing services is spreading by wordof-mouth: 40% of new customers are referred by the existing 5,000-customer installed base. `[ps=5]`
- This study analyzes ==> picture [442 x 39] intentionally omitted <== using Aberdeen Group research material and prepared extraction artifacts. `[ps=0]`
