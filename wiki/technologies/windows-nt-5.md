---
title: "Microsoft Windows NT 5.0 / Windows 2000"
slug: "windows-nt-5"
page_type: "technology"
tags: ["type/technology", "category/operating-system", "era/1998-2010"]
tier: 2
source_csv: "_master_technologies.csv"
tech_id: "windows-nt-5"
category: "operating-system"
vendor: "Microsoft"
era: "1998-2010"
lifecycle_at_study: "emerging"
lifecycle_current: "legacy-supplanted"
occurrence_count: 2
prescience_max: 5.0
prescience_mean: 3.69
prescience_obs_count: 13
---

# Microsoft Windows NT 5.0 / Windows 2000

> Windows NT 5.0 renamed Windows 2000; released Feb 2000; IntelliMirror, Active Directory, Group Policy; Zero Admin cornerstone


## Top observations

- NT 5.0 will not ship until late 1999 at earliest; may be delayed by 6-12 months `[ps=5]` — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- Windows 2000 RTM December 15 1999; GA February 17 2000 — consistent with Aberdeen's late-1999 prediction `[ps=5]` — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- NT market penetration at 25%; new-market share at 40% and growing by some estimates `[ps=4]` — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- Short-term developer productivity will go down sharply; long-term benefits from integrated directory services and enhanced file services `[ps=4]` — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- Benefits will be slow to arrive and unevenly distributed across enterprise types; careful planning is mandatory `[ps=4]` — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- NT 5.0 with 40M lines of code and CE integration positions as platform for future of all Windows releases — corporate server to embedded `[ps=4]` — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- Microsoft Windows NT 5.0 delays were the central risk to the Zero Administration initiative as of August 1998; the release was rebranded Windows 2000 and shipped Feb 17 2000 — roughly 18 months after this article, validating Kastner-era skepticism about the schedule. `[ps=4]` — [[study-systems-management-delayed-promise-1bc587]]
- Should have greatest OS-upgrade impact on all types of enterprises at least since the heyday of the mainframe; and perhaps the greatest impact ever `[ps=3]` — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- NT 5.0 has as much as 40 million lines of code `[ps=3]` — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- Enterprise Edition with enhanced scalability projected within a year of NT 5.0 FCS; 64-bit version planned for Intel IA-64 when Merced reaches mass market `[ps=3]` — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- NT 5.0 Workstation requires 64 MB RAM; users upgrading desktops will need to rebuild file systems — major inconvenience multiplied across enterprise's thousands of desktops `[ps=3]` — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- NT 5.0 storage management capabilities likely to be a pleasant surprise; one-stop storage shopping and reduced labor burden `[ps=3]` — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- Early implementers who have not preplanned and pretested will face major enterprise-implementation problems similar to NT 3.5.1 and NT 4.0 `[ps=3]` — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- NT 5.0 improvements will make it comparable to some midrange environments; but will not fully arrive until 1 year (best case) or 2 years (Aberdeen worst-case) post-FCS — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- Microsoft best game in town for Internet/Intranet scaling applications; NT 5.0 will strengthen but not fully deliver on Internet infrastructure promise — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- Windows 2000 achieved significant enterprise adoption but was largely superseded by Windows XP (Oct 2001) and Server 2003 within 3 years; confirming Aberdeen's prediction of uneven and slow benefits — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
- NT 5.0 release will occur in frantic atmosphere of Year 2000 concerns and Internet hype; IS should not allow these issues to distract from NT 5.0 migration planning — [[study-1998-microsoft-windows-nt-5-0-a-planning-guide-cd9d08]]
